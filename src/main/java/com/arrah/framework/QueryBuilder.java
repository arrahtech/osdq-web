...
//			range_sel_query += " ROWS ONLY";
    ...
    public String getJoinQuery(Hashtable<String, String> ruleDetails) {
        ...
        switch (ruleDetails.get("Database_Type")) {
            case "Oracle JDBC Client":
            case "Oracle Windows Bridge":
                if( cond.length > 3 ) {
                ...
                break;
            case "Mysql JDBC Client":
            case "Mysql Windows Bridge":
                if( cond.length > 3 ) {
                    j_query = getMultiJoinQuery(ruleDetails, ruleDetails.get("Database_Type"));
                } else {
                    String temp;
            ...
            case "SQLServer JDBC Client":
            case "SQLServer Windows Bridge":
                if( cond.length > 3 ) {
                    j_query = getMultiJoinQuery(ruleDetails, ruleDetails.get("Database_Type"));
                } else {
                    String temp;
            ...
            case "Postgres JDBC Client":
                ...
                break;
            case "DB2 JDBC Client":
            case "Hive JDBC Client":
                if( cond.length > 3 ) {
                    j_query = getMultiJoinQuery(ruleDetails, ruleDetails.get("Database_Type"));
                } else {
                    String temp;
                ...
                break;
            case "Others (JDBC Bridge)":
                break;
            case "Others (Windows Bridge)":
                break;
        }
        return j_query;
    }
    ...
    public List<String> get_all_tables(Hashtable<String, String> dbDetails) {
        ...
        try {
            ...
            switch (dbDetails.get("Database_Type")) {
                case "Oracle JDBC Client":
                case "Oracle Windows Bridge":
               Rdbms_NewConn dbmsConn1 =  new Rdbms_NewConn(dbDetails);
                    dbmsConn1.openConn();
                    System.out.println("Connected to " + dbDetails.get("Database_ConnectionName"));
                    rs = dbmsConn1.execute(SqlUserTables.ORACLE_USERTABLES);
                    while (rs.next()) {
                        db_tables.add(rs.getString(1));
                    }
                    dbmsConn1.closeConn();
                    rs.close();
                    break;
                case "Mysql JDBC Client":
                case "Mysql Windows Bridge":
                    dbmsConn1 = new Rdbms_NewConn(dbDetails);
                    dbmsConn1.openConn();
                    DatabaseMetaData md = dbmsConn1.getMetaData();
                    rs = md.getTables(null, null, "%", null);
                    System.out.println("Connected to " + dbDetails.get("Database_ConnectionName"));
                    md = dbmsConn1.getMetaData();
                    rs = md.getTables(null, null, "%", null);
                    while (rs.next()) {
                        db_tables.add(rs.getString(3));
                    }
                    dbmsConn1.closeConn();
                    rs.close();
                    break;
                ...
                case "SQLServer Windows Bridge":
                    ...
                    dbmsConn1.openConn();
                    System.out.println("Connected to " + dbDetails.get("Database_ConnectionName"));
                    rs = dbmsConn1.execute(SqlUserTables.MSSQL_USERTABLES);
                    while (rs.next()) {
                        db_tables.add(rs.getString(1));
                    }
                    dbmsConn1.closeConn();
                ...
                case "Access Windows Bridge":
                    ...
                    dbmsConn1.openConn();
                    System.out.println("Connected to " + dbDetails.get("Database_ConnectionName"));
                    rs = dbmsConn1.execute(SqlUserTables.MSACCESS_USERTABLES);
                    while (rs.next()) {
                        db_tables.add(rs.getString(1));
                    }
                    dbmsConn1.closeConn();
                ...
                case "Postgres JDBC Client":
                    ...
                    dbmsConn1.openConn();
                    System.out.println("Connected to " + dbDetails.get("Database_ConnectionName"));
                    rs = dbmsConn1.execute(SqlUserTables.POSTGRESS_USERTABLES);
                    while (rs.next()) {
                        db_tables.add(rs.getString(1));
                    }
                    dbmsConn1.closeConn();
                ...
                case "DB2 JDBC Client":
                    ...
                    dbmsConn1.openConn();
                    System.out.println("Connected to " + dbDetails.get("Database_ConnectionName"));
                    rs = dbmsConn1.execute(SqlUserTables.DB2_USERTABLES);
                    while (rs.next()) {
                        db_tables.add(rs.getString(1));
                    }
                    dbmsConn1.closeConn();
                ...
                case "Hive JDBC Client":
                    ...
                    dbmsConn1.openConn();
                    System.out.println("Connected to " + dbDetails.get("Database_ConnectionName"));
                    rs = dbmsConn1.execute(SqlUserTables.HIVE_USERTABLES);
                    while (rs.next()) {
                        db_tables.add(rs.getString(1));
                    }
                    dbmsConn1.closeConn();
                ...
                case "Informix JDBC Client":
                    ...
                    dbmsConn1.openConn();
                    System.out.println("Connected to " + dbDetails.get("Database_ConnectionName"));
                    rs = dbmsConn1.execute(SqlUserTables.INFORMIX_USERTABLES);
                    while (rs.next()) {
                        db_tables.add(rs.getString(1));
                    }
                    dbmsConn1.closeConn();
                ...
                case "Splice JDBC Client":
                    ...
                    dbmsConn1.openConn();
                    System.out.println("Connected to " + dbDetails.get("Database_ConnectionName"));
                    rs = dbmsConn1.execute(SqlUserTables.SPLICE_USERTABLES);
                    while (rs.next()) {
                        db_tables.add(rs.getString(1));
                    }
                    dbmsConn1.closeConn();
    ...
    public List<String> get_all_cols(Hashtable<String, String> dbDetails, String t_name) {
        ...
        try {
            ...
            if (dbDetails.get("Database_Type").equals("Oracle JDBC Client") || dbDetails.get("Database_Type").equals("Oracle Windows Bridge")) {
            } else if (dbDetails.get("Database_Type").equals("Mysql JDBC Client") || dbDetails.get("Database_Type").equals("Mysql Windows Bridge")) {
            ...
            } else if (dbDetails.get("Database_Type").equals("SQLServer JDBC Client") || dbDetails.get("Database_Type").equals("SQLServer Windows Bridge")) {
                dbmsConn = new Rdbms_NewConn(dbDetails);
                ...
                rsCol.close();
                dbmsConn.closeConn();
            } else if (dbDetails.get("Database_Type").equals("Access Windows Bridge")) {
            } else if (dbDetails.get("Database_Type").equals("Postgres JDBC Client")) {
            } else if (dbDetails.get("Database_Type").equals("DB2 JDBC Client")) {
            } else if (dbDetails.get("Database_Type").equals("Hive JDBC Client")) {
            } else if (dbDetails.get("Database_Type").equals("Others (JDBC Bridge)")) {
            } else if (dbDetails.get("Database_Type").equals("Others (Windows Bridge)")) {
            } else if (dbDetails.get("Database_Type").equals("Splice JDBC Client")) {
                dbmsConn = new Rdbms_NewConn(dbDetails);
                ...
                dbmsConn.closeConn();
            } else if (dbDetails.get("Database_Type").equals("Informix JDBC Client")) {
                dbmsConn = new Rdbms_NewConn(dbDetails);
	...
	public String getVolumeQuery(String dsn) {
